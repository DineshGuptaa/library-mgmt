...
export default function Login() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (credentialResponse) => {
      ...
      setLoading(true)
      try {
        const fn = role === 'MEMBER' ? authApi.registerMemberGoogle : authApi.registerAuthorGoogle
        const res = await fn({ token: (credentialResponse as any).access_token })
        login(res.data)
        navigate('/dashboard')
      } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          ...
          {role !== 'ADMIN' && (
            <>
              ...
              <button type="button" onClick={() => googleLogin()}
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 border border-gray-300 rounded-lg py-2.5 hover:bg-gray-50 disabled:opacity-50 transition-colors">
                ...
                <span className="text-sm font-medium text-gray-600">{loading ? 'Signing in...' : `Sign in with Google`}</span>
              </button>
            </>
          )}
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link to="/register" className="text-indigo-600 font-medium hover:text-indigo-700">Register</Link>
          </p>
        </div>
      </div>
    </div>
