  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (credentialResponse) => {
      ...
      try {
        ...
        const res = await fn({ token: (credentialResponse as any).id_token })
        login(res.data)
        navigate('/dashboard')
      } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          ...
          <button type="button" onClick={() => googleLogin()}
            ...
            <span className="text-sm font-medium text-gray-600">{loading ? 'Signing in...' : `Sign up with Google`}</span>
          </button>
