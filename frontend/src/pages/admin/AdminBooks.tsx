...
export default function AdminBooks() {
  ...
  const fetchBooks = () => {
    setLoading(true)
    booksApi.getAll({ page, limit: LIMIT })
      .then((res) => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">View, add, and update books</p>
        </div>
        <button
          onClick={openAddForm}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid sm:grid-cols-3 gap-5">
              <SearchableSelect
                ...
                options={authors.map((a) => ({ id: a.id, label: a.name || `Author #${a.id}` }))}
                value={form.authorId}
                onChange={(v) => setForm((prev) => ({ ...prev, authorId: v }))}
              />
              <SearchableSelect
                label="Publisher"
